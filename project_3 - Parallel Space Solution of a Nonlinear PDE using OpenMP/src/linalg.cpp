...
#pragma omp parallel for default(none) shared(x, N, value)
    ...
    {
#pragma omp parallel for default(none) shared(x, y, N, alpha)
        for (int i = 0; i < N; i++)
        {
            y[i] += alpha * x[i];
        }
    ...
    {
#pragma omp parallel for default(none) shared(x, y, l, r, N, alpha)
        for (int i = 0; i < N; i++)
        {
    ...
    void hpc_scaled_diff(Field &y, const double alpha,
                         Field const &l, Field const &r, const int N)
    {
#pragma omp parallel for default(none) shared(y, l, r, N, alpha)
        for (int i = 0; i < N; i++)
        {
            y[i] = alpha * (l[i] - r[i]);
        }
...
#pragma omp parallel for default(none) shared(x, y, N, alpha)
    ...
    {
#pragma omp parallel for default(none) shared(x, y, z, N, alpha, beta)
        for (int i = 0; i < N; i++)
        {
