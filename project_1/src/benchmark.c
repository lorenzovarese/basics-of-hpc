...
// On M2 Pro (Performance core)
// 3.49 GHz * 4 vector width * 1 flops for FMA = 13.96 GF/s
#define MAX_SPEED 13.96
...
int main(int argc, char **argv)
{
  ...
  int test_sizes[] =
      ...
      /*{31,32,33,63,64,65,95,96,97,127,128,129,159,160,161,191,192,193,223,224,225,255,256,257,287,288,289,319,320,321,
      351,352,353,383,384,385,415,416,417,447,448,449,479,480,481,511,512,513,543,544,545,575,576,577,607,608,609,639,640,
      641,671,672,673,703,704,705,735,736,737,767,768,769,799,800,801,831,832,833,863,864,865,895,896,897,927,928,929,959,
  ...
  for (int isize = 0; isize < sizeof(test_sizes) / sizeof(test_sizes[0]); ++isize)
  {
    ...
    for (int i = 0; i < n * n; ++i)
      if (C[i] > 0)
      {
        printf("Print matrix that triggers the error:\n");
        for (int i = 0; i < n * n; ++i)
        {
          if (i % n == 0)
            printf("\n");
