...
int main(int argc, char **argv)
{
       ...
       for (j = 0; j < IMAGE_HEIGHT; j++)
       {
              ...
              for (i = 0; i < IMAGE_WIDTH; i++)
              {
                     ...
                     // |z|<2 but to avoid square root in module computing I use |z|^2 < 4
                     for (n = 1; (x2 + y2) < 4 && n < MAX_ITERS; n++)
                     {
                            // z = zˆ2 + c (x=Re(z) and y=Im(z))
                            y = 2 * x * y + cy; // compute the imaginary part first
                            x = x2 - y2 + cx;
                            // Computing the square for the next iteration
                            x2 = x * x;
                            y2 = y * y;
                     }
