...
int main (int argc, char** argv)
{
	...
	MPI_Type_vector(d.nx * d.ny, 1, 1, MPI_INT, &plot_datatype);                     
  	MPI_Type_commit(&plot_datatype); 
	...
	if (mpi_rank != 0)
	{
		...
		MPI_Send(c, 1,  plot_datatype, 0, 0, p.comm);
	}
	/****************************************************************************/
	// Write the image
	if (mpi_rank == 0)
	{
		...
		for (int proc = 1; proc < mpi_size; proc++)
		{
			...
			MPI_Irecv(c, 1, plot_datatype, proc, 0, p.comm, &request);
